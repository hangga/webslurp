...
function renderList() {
  ...
  filtered.forEach((log, idx) => {
    ...
    entry.className = 'log-entry' + (selectedIdx === realIdx ? ' active' : '');
...
function selectLog(idx) {
  ...
  selectedIdx = idx;
  editingIdx = null; // keluar dari mode edit
  renderList();
  renderDetail(idx);
}
...
async function refresh() {
  ...
  logs = result.logs || [];
  renderList();
  if (selectedIdx !== null && !logs[selectedIdx]) {
    selectedIdx = null;
  }
  if (selectedIdx !== null) {
...
async function autoAttach() {
  try {
    ...
    const res = await chrome.runtime.sendMessage({ action: 'attach', tabId: tabs[0].id });
    if (res?.success) {
      isAttached = true;
